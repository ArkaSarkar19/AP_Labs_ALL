...
public abstract class Hero {
	...
	protected int level;
	protected ArrayList<sidekick> sidekicks = new ArrayList<sidekick>();
	protected sidekick currsidekick = null;
...
//		if(currsidekick!=null) {
//			currsidekick.levelup(monster.getlevel()*20);
//		}
		currsidekick = null;
		this.HP = this.maxHP;
		this.specialmove = 0;
	...
	public void checkxp(monster monster) {
		this.killedmonster(monster);
	}
	public void addsidekick(sidekick s, int amount) {
		if(amount > this.XP) {
			System.out.println("Not enought XP sorry ");
		}
		else {
			this.XP-=amount;
			sidekicks.add(s);
			System.out.println("You bought a sidekick : " +  s.gettype());
			System.out.println("XP of sidekick is " + s.getXP() );
			System.out.println("Attack of sidekick is " + s.getattack());
		}
	}
	public sidekick getsidekick() {
		Collections.sort(sidekicks, new xpcomparator());
		currsidekick = sidekicks.get(0);
		return currsidekick;
	}
...
final class Warrior extends Hero{
	...
	public void specialmove(monster monster) {
		if(!this.checkspecialmove()) {
			System.out.println("Wrong move can't do");	
		}
		else {	
...
final class Mage extends Hero{
	...
	public void specialmove(monster monster) {
		...
		this.specialmove = this.moves;
		monster.specialmove(this);}
		else System.out.println("Wrong move can't do");
...
final class Healer extends Hero{
	...
	private void heal() {
		if(this.moves!=0 && this.moves <= this.specialmove+3) {
